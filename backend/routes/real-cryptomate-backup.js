...
// Función para traer transacciones de una tarjeta específica
const fetchTransactionsFromCard = async (cardId, fromDate = '2024-01-01', toDate = '2025-01-25', page = 1) => {
  const { exec } = require('child_process');
  const { promisify } = require('util');
  const execAsync = promisify(exec);
  
  try {
    const curlCommand = `curl --location 'https://api.cryptomate.me/cards/transactions/${cardId}/search?from_date=${fromDate}&to_date=${toDate}&operations=TRANSACTION_APPROVED,TRANSACTION_REJECTED,TRANSACTION_REVERSED,TRANSACTION_REFUND,WALLET_DEPOSIT&size=100&page_number=${page}' --header 'x-api-key: api-45f14849-914c-420e-a788-2e969d92bd5d' --header 'Content-Type: application/json' --header 'Cookie: JSESSIONID=7216B94569B249C7E74CF7409C99C656'`;
    
    console.log(`🚀 Fetching transactions for card ${cardId} (page ${page})...`);
    const { stdout, stderr } = await execAsync(curlCommand);
...
const convertCryptoMateTransactionToNano = (cryptoTransaction, cardId, userId) => {
  ...
  return {
    ...
    userId: userId,
    cardId: cardId,
    name: cryptoTransaction.merchant_name || 'Unknown Transaction',
    ...
    credit: cryptoTransaction.operation === 'WALLET_DEPOSIT' || cryptoTransaction.operation === 'TRANSACTION_REFUND',
...
// Endpoint para importar transacciones de una tarjeta específica
router.post('/import-transactions/:cardId', async (req, res) => {
  try {
    const { cardId } = req.params;
    const { fromDate = '2024-01-01', toDate = '2025-01-25', maxPages = 5 } = req.body;
    ...
    while (currentPage <= maxPages) {
      try {
        ...
        const cryptoTransactions = await fetchTransactionsFromCard(cardId, fromDate, toDate, currentPage);
        ...
        for (const cryptoTransaction of cryptoTransactions) {
          try {
            ...
            if (!existingTransaction) {
              ...
              console.log(`✅ Imported transaction: ${nanoTransaction._id} - ${nanoTransaction.name}`);
            } else {
              // Actualizar transacción existente
              Object.assign(existingTransaction, nanoTransaction);
              await existingTransaction.save();
              updatedTransactions++;
              console.log(`🔄 Updated transaction: ${nanoTransaction._id} - ${nanoTransaction.name}`);
            }
